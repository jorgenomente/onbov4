...
-- Post-MVP 3 / Configuracion del bot — Sub-lote C (C.1 + C.2)
-- Objetivo: final_evaluation_configs = INSERT-only (append-only real) + RPC segura para crear nueva config
-- NO UI. NO cambios de engine. NO cambios de schema (solo trigger + funcion).
...
-- Si ya existe con misma firma, esto solo la redefine (misma intencion).
...
-- - y ademas reforzamos con checks por current_role/current_org_id
...
-- Permitir ejecutar la RPC desde sesion autenticada (RLS sigue aplicando).
